...
`CommonsChunkPlugin` 插件，是一个可选的功能， 用于生成一个独立chunk，这个chunk由多个入口 `chunk` 的公共模块构成。将公共模块抽取到一个文件里面能够让这些模块在最开始的时候加载一次，便存到缓存中供后续使用。这个能带来速度上的提升，因为浏览器会迅速将公共的代码从缓存中取出来，而不是每次访问一个新页面时，再去加载一个更大的文件。
